	...
	return new Promise((resolve) => {
		setTimeout(() => {
			const nearestDestinations = destinations
				...
				.sort((a, b) => {
					const distanceA =
						Math.abs(a.latitude - selectedDestination.latitude) +
						Math.abs(a.longitude - selectedDestination.longitude);
					const distanceB =
						Math.abs(b.latitude - selectedDestination.latitude) +
						Math.abs(b.longitude - selectedDestination.longitude);
